...
import { Calendar as CalendarIcon, Save, MapPin, Clock, Users } from "lucide-react"
...
export function EditEventModal({ open, onOpenChange, onEventUpdate, event }: EditEventModalProps) {
  ...
  useEffect(() => {
    if (event) {
      setFormData({
        ...
        duracao: event.duracao || "120",
        local: event.local || "",
        responsavel: event.responsavel || "",
        descricao: event.descricao || "",
        participantesEsperados: event.participantesEsperados?.toString() || "20",
        status: event.status || "Agendado"
      })
      
      if (event.data) {
        try {
          const parsedDate = parse(event.data, "yyyy-MM-dd", new Date())
          setDate(parsedDate)
        } catch (error) {
